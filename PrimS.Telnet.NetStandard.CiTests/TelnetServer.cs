namespace PrimS.Telnet.CiTests
{
  ...
  public class TelnetServer : System.Net.Sockets.Socket
  {
      ...
      : base(AddressFamily.InterNetwork, SocketType.Stream, ProtocolType.Tcp)
    {
      ...
      task.Wait();
      IPHostEntry ipHostInfo = task.Result;
    ...
    private void SpinListen()
    {
      try
      {
        ...
        while (this.IsListening)
        {
          ...
          using (Socket handler = this.Accept())
          {
            data = null;
    ...
    private bool IsResponseReceived(string currentResponse, string responseAwaited)
    {
      ...
      {
        System.Diagnostics.Debug.WriteLine("{0} response received", responseAwaited);
        Console.WriteLine("{0} response received", responseAwaited);
        return true;
      }
      else
      {
        System.Diagnostics.Debug.WriteLine("Waiting for {1} response, received {0}", currentResponse, responseAwaited);
